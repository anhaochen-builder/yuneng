    ...
    <div class="grid-2col">
      ...
      <div class="tech-card">
        <div class="card-header">
          <h4>📡 设备列表</h4>
          <el-input v-model="searchText" placeholder="搜索设备..." size="small" clearable style="width:200px" :prefix-icon="'Search'" />
        </div>
        ...
        <el-table :data="filteredDevices" size="small" v-loading="loading" highlight-current-row @row-click="queryDevice" max-height="420">
          ...
          <el-table-column prop="device_type" label="类型" width="90">
            <template #default="{ row: r }">
              <el-tag size="small" effect="plain">{{ r.device_type === 'wind_turbine' ? '风机' : r.device_type === 'inverter' ? '逆变器' : r.device_type === 'transformer' ? '变压器' : r.device_type === 'combiner_box' ? '汇流箱' : r.device_type }}</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="temp" label="温度(°C)" width="85">
            <template #default="{ row: r }">
              <span class="font-digital" :style="{color: (r.temp||0) > 80 ? '#ff4d4f' : (r.temp||0) > 65 ? '#ff9c40' : '#52c41a'}">{{ r.temp }}°C</span>
            </template>
          </el-table-column>
      ...
      <div>
        <div class="tech-card" v-if="selected" v-loading="querying">
          ...
          <div class="quick-actions" style="margin-top:12px">
            <el-button size="small" type="primary" @click="$router.push(`/diagnostic?q=${selected.name}故障排查`)">
              🔧 智能诊断
            </el-button>
